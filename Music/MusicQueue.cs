...
public class MusicQueue(GuildMusicData guildMusic) {
    ...
    private async Task<Track> selectNextSong(Artist artist, string artistName) {
        ...
        var tracks_ = await GuildMusicData.getTrackAsync(Program.LavalinkNode, randomFile);
        if (tracks_ == null) {
            // retry if not found
            goto beginning;
        }
    ...
    public async Task Player_PlaybackFinished(object sender, TrackEndedEventArgs e) {
